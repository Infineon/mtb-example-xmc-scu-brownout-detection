# XMC&trade; MCU: Brownout detection

This code example demonstrates brownout detection by reducing the power supply voltage from 3.3 V to the defined voltage threshold using brownout detection available in XMC&trade; MCU devices.
...
This example requires an additional power supply to reduce the power supply of the board.
...
The following example will clone the "[Brownout Detection](https://github.com/Infineon/mtb-example-xmc-brownout-detection)" application with the desired name "BrownoutDetection" configured for the *KIT_XMC47_RELAX_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC47_RELAX_V1 --app-id mtb-example-xmc-brownout-detection --user-app-name BrownoutDetection --target-dir "C:/mtb_projects"
   ```
...
3. Disconnect the board from the micro-USB cable.

4. Connect a power supply to VDD3.3 and GND on the board.

5. Decrease the supply voltage from 3.3 V to 3.0 V slowly.

6. Confirm that the kit LED does not glow anymore.
...
The goal of this example is to implement a code to detect brownout.

The `POWER_EnableMonitor()` function detects brownout. The arguments to this function are the supply voltage threshold and the interval value for comparison with VDDP, expressed in cycles of system clock.

Initially, the user LED is set to HIGH. The LED pin is configured in **Device Configurator** under **Pins** by the **Pin 1.0** personality. The corresponding pin label is CYBSP_USER_LED.

<img src="images/pin.png" alt="1" style="zoom:80%;" />

When the supply voltage reaches the threshold defined in the `POWER_EnableMonitor()` function, the output of the LED will be set to LOW to indicate that brownout is detected.
...
Document title: *CE232799* – *XMC&trade; MCU: Brownout detection*
